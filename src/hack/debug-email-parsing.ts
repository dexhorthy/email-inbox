...
import { createGmailClient, parseEmailBody, convertToMarkdown } from "../emailParser"

async function debugEmailParsing() {
  console.log("🔍 Debugging email parsing...")

  // Load test data
  const testDataPath = path.join(process.cwd(), "src", "test", "data", "test-message-ids.json")
    ...
    payload.parts.forEach((part, i) => {
      ...
      if (part.parts) {
        part.parts.forEach((subPart, j) => {
          ...
          console.log(`         Has Body Data: ${subPart.body?.data ? "YES" : "NO"}`)
